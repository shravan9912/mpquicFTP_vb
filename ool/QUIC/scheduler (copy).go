...
const DumpDataFile = "/home/mosaic/edge-computing-mpquic/message/DumpDataPointer"
const CheckNotificationFile = "/home/mosaic/edge-computing-mpquic/message/CheckNotificationPointer"
const DetectAgentPath = "/home/mosaic/edge-computing-mpquic/output/"
const DumpDataInterval = 600
const CheckNotificationInterval = 200 
const RetrainingPacketNum = 1000
const RetrainingPacketNumFile = "/home/mosaic/edge-computing-mpquic/message/RetrainingPacketNum"
...
type scheduler struct {
	...
	statevector [50000]types.Vector
	packetvector [50000]uint64
	//rewardvector [6000]types.Output
	actionvector [50000]int
	recordDuration [50000]types.Output
	...
	featureone [50000]float64
	featuretwo [50000]float64
	featurethree [50000]float64
	featurefour [50000]float64
	featurefive [50000]float64
	featuresix [50000]float64
	...
	Redundant bool
        ...
        DumpDataCounter uint64
        DumpDataNum int
        CheckNotificationCounter uint64
        CheckNotificationNum int
        ReTrainingPacketCounter int
}
...
func (sch *scheduler) setup() {
	...
	sch.waiting = 0
        ...
        fileRetrain, _ := os.Open(RetrainingPacketNumFile)
        fmt.Fscanln(fileRetrain, &sch.ReTrainingPacketCounter)
        fileRetrain.Close()

	//Read lin to buffer
	file, err := os.Open("./lin")
	if err != nil {
    	panic(err)
	}
	...
	file.Close()
        ...
        sch.utilizationstart = time.Now()

	//TODO: expose to config
	sch.DumpPath = "/tmp/"
	sch.dumpAgent.Setup()
        sch.Redundant = false
        sch.dumpDetection.Setup()
...
func (sch *scheduler) selectPathDoubleBandit(s *session, hasRetransmission bool, hasStreamRetransmission bool, fromPth *path) *path {
	// XXX Avoid using PathID 0 if there is more than 1 path
	if sch.quotas == nil {
		sch.setup()
	}
	...
	for pathID, path := range s.paths {
		if pathID != protocol.InitialPathID {
			if bestPath == nil {
				bestPath = path
			} else if path.sentPacketHandler.GetBytesInFlight() < bestPath.sentPacketHandler.GetBytesInFlight() {
				secondBestPath = bestPath
				bestPath = path
			} else if secondBestPath == nil || path.sentPacketHandler.GetBytesInFlight() < secondBestPath.sentPacketHandler.GetBytesInFlight() {
				secondBestPath = path
			}
		}
	}
	...
	if bestPath != nil && secondBestPath != nil {
		for sch.episoderecord < sch.record {
			...
			if sch.packetvector[sch.episoderecord] <= cureNum {
				curereward = float64(protocol.DefaultTCPMSS) / float64(time.Since(sch.zz[sch.episoderecord])/10000)
			} else {
				break
			}
			// Update Aa, ba
		...
		sch.featureseven[sch.record] = feature.At(6, 0)
...
func (sch *scheduler) selectPathDeployBandit(s *session, hasRetransmission bool, hasStreamRetransmission bool, fromPth *path) *path {
	// XXX Avoid using PathID 0 if there is more than 1 path
	if len(s.paths) <= 1 {
		if !hasRetransmission && !s.paths[protocol.InitialPathID].SendingAllowed() {
			return nil
	...
	for pathID, path := range s.paths{
		...
		if 0 < float64(lsecondLowerRTT) && 0 < float64(llowerRTT) {
			feature.Set(0, 0, cwndBest/float64(llowerRTT)*100)
			feature.Set(2, 0, float64(BSend)/float64(llowerRTT))
			feature.Set(4, 0, inflightf/float64(llowerRTT)*100)
			feature.Set(1, 0, inflights/float64(lsecondLowerRTT)*100)
			feature.Set(3, 0, float64(BSend)/float64(lsecondLowerRTT))
			feature.Set(5, 0, cwndSecond/float64(lsecondLowerRTT)*100)
			if hasRetransmission {
				feature.Set(6, 0, float64(1))
			} else {
				feature.Set(6, 0, float64(0))
			}
		} else {
			feature.Set(0, 0, 0)
			feature.Set(2, 0, 0)
...
func (sch *scheduler) selectPathRound(s *session, hasRetransmission bool, hasStreamRetransmission bool, fromPth *path) *path {
	// XXX Avoid using PathID 0 if there is more than 1 path
	if len(s.paths) <= 1 {
		if !hasRetransmission && !s.paths[protocol.InitialPathID].SendingAllowed() {
			return nil
	...
	if len(availablePaths) == 0 {
                sch.quotas[protocol.PathID(1)] = 0
                sch.quotas[protocol.PathID(3)] = 0
		return nil
	}

        if len(availablePaths) == 2 {
		if sch.quotas[protocol.PathID(1)] <= sch.quotas[protocol.PathID(3)] {
                    return s.paths[protocol.PathID(1)]
                } else {
                    return s.paths[protocol.PathID(3)]
                }
	}
...
func (sch *scheduler) selectPathDQNAgent(s *session, hasRetransmission bool, hasStreamRetransmission bool, fromPth *path) *path {
	...
	//if action == 1{
	//	return s.paths[availablePaths[3]]
	//}
...
func (sch *scheduler) selectPathFullLin(s *session, hasRetransmission bool, hasStreamRetransmission bool, fromPth *path) *path {
	// XXX Avoid using PathID 0 if there is more than 1 path
	if len(s.paths) <= 1 {
		if !hasRetransmission && !s.paths[protocol.InitialPathID].SendingAllowed() {
			return nil
...
func (sch *scheduler) selectPath(s *session, hasRetransmission bool, hasStreamRetransmission bool, fromPth *path) *path {
	...
	}else if sch.SchedulerName == "redundant"{
		return sch.selectPathRedundant(s, hasRetransmission, hasStreamRetransmission, fromPth)
	}else if sch.SchedulerName == "deploy"{
		return sch.selectPathDeployBandit(s, hasRetransmission, hasStreamRetransmission, fromPth)
                //return sch.selectPathPeek(s, hasRetransmission, hasStreamRetransmission, fromPth)
	}else if sch.SchedulerName == "double"{
		return sch.selectPathDoubleBandit(s, hasRetransmission, hasStreamRetransmission, fromPth)
	...
	}else if sch.SchedulerName == "fullLin" {
		return sch.selectPathFullLin(s, hasRetransmission, hasStreamRetransmission, fromPth)
	}else if sch.SchedulerName == "primary" {
		return sch.selectFirstPath(s, hasRetransmission, hasStreamRetransmission, fromPth)
	}else if sch.SchedulerName == "robin" {
		return sch.selectPathRoundRobin(s, hasRetransmission, hasStreamRetransmission, fromPth)
	}else if sch.SchedulerName == "round" {
		return sch.selectPathRound(s, hasRetransmission, hasStreamRetransmission, fromPth)
	}else{
		// Default, rtt
		return sch.selectPathLowLatency(s, hasRetransmission, hasStreamRetransmission, fromPth)
	}
...
func (sch *scheduler) performPacketSending(s *session, windowUpdateFrames []*wire.WindowUpdateFrame, pth *path) (*ackhandler.Packet, bool, error) {
	...
	for _, f := range windowUpdateFrames {
        ...
        // packet utilization per second log
        utiliztionelapsed := time.Since(sch.utilizationstart)
        if utiliztionelapsed > time.Second {
                sch.utilizationstart = time.Now()
                for pathID, pth := range s.paths {
                    if pathID == protocol.PathID(1){
		        sntPkts, _, _ := pth.sentPacketHandler.GetStatistics()
			utils.Infof("Path %x: sent %d", pathID, sntPkts-sch.utilfirst)
                        sch.utilfirst = sntPkts
		    }
                    if pathID == protocol.PathID(3){
		        sntPkts, _, _ := pth.sentPacketHandler.GetStatistics()
			utils.Infof("Path %x: sent %d", pathID, sntPkts-sch.utilthrid)
                        sch.utilthrid = sntPkts
		    }
                }
        }
	...
	for _, frame := range packet.frames {
		...
		case *wire.StreamFrame:
			if frame.FinBit {
				...
				s.pathsLock.RLock()
				firstPathPackets, secondPathPackets := uint64(0), uint64(0)
				utils.Infof("Info for stream %x of %x", frame.StreamID, s.connectionID)
				for pathID, pth := range s.paths {
					sntPkts, sntRetrans, sntLost := pth.sentPacketHandler.GetStatistics()
					rcvPkts := pth.receivedPacketHandler.GetStatistics()
					utils.Infof("Path %x: sent %d retrans %d lost %d; rcv %d rtt %v stdev %v", pathID, sntPkts, sntRetrans, sntLost, rcvPkts, pth.rttStats.SmoothedRTT(), pth.rttStats.MeanDeviation())
                    if pathID == protocol.PathID(1){
						firstPathPackets = sntPkts
					}
                    if pathID == protocol.PathID(3){
						secondPathPackets = sntPkts
					}
                                                
					// TODO: Remove it
					utils.Infof("Congestion Window: %d", pth.sentPacketHandler.GetCongestionWindow())
					//utils.Infof("theta: %v, %v, %v, %v, %v, %v", sch.featureone[sch.record-1], sch.featuretwo[sch.record-1], sch.featurethree[sch.record-1], sch.featurefour[sch.record-1], sch.featurefive[sch.record-1], sch.featuresix[sch.record-1])
				...
				utils.Infof("Dump: %t, Training:%t, scheduler:%s", sch.DumpExp, sch.Training, sch.SchedulerName)
                                os.Remove(DumpDataFile)
				os.Create(DumpDataFile)
                                fileend, _ := os.OpenFile(DumpDataFile, os.O_WRONLY, 0600)
                                fileendsymbol := int(-1)
                                fmt.Fprintf(fileend, "%d", fileendsymbol)
                                fileend.Close()
                                os.Remove(RetrainingPacketNumFile)
				os.Create(RetrainingPacketNumFile)
                                fileretrainingnum, _ := os.OpenFile(RetrainingPacketNumFile, os.O_WRONLY, 0600)
                                fmt.Fprintf(fileretrainingnum, "%d", sch.ReTrainingPacketCounter)
                                fileretrainingnum.Close() 
				...
				if sch.DumpExp && !sch.Training && sch.SchedulerName == "dqnAgent"{
					...
					sch.dumpAgent.CloseExperience(uint64(s.connectionID))
                                ...
                                utils.Infof("%d %d", firstPathPackets, secondPathPackets)
				s.pathsLock.RUnlock()
				//Write lin parameters
				os.Remove("./lin")
				os.Create("./lin")
				file2, _ := os.OpenFile("./lin", os.O_WRONLY, 0600)
				for i := 0; i < banditDimension; i++ {
					for j := 0; j < banditDimension; j++ {
