...
func LinReward(sch *scheduler, s *session) (int, []*path){
	packetNumber := make(map[protocol.PathID]uint64)
	retransNumber := make(map[protocol.PathID]uint64)
	...
	BSend, _ := s.flowControlManager.SendWindowSize(protocol.StreamID(5))
        stateone := NormalizeTimes(sRTT[firstPath])
	statetwo := NormalizeTimes(sRTT[secondPath])
	statethree := types.Output(cwnd[firstPath])/types.Output(protocol.DefaultTCPMSS)/300
	statefour := types.Output(cwnd[secondPath])/types.Output(protocol.DefaultTCPMSS)/300
	statefive := cwndlevel[firstPath]
	statesix := cwndlevel[secondPath]
	stateseven := types.Output(BSend)/types.Output(protocol.DefaultTCPMSS)/30000
	armzero := (7.53393198)*stateone + (3.0664253)*statetwo + (0.741272808)*statethree + (18.475318)*statefour + (5.26049134)*statefive + (1.41524505)*statesix + (36.3028542)*stateseven 
        armone := (4.60796698)*stateone + (-14.5457596)*statetwo + (27.5014974)*statethree + (16.6146045)*statefour + (-0.302466900)*statefive + (9.90184211)*statesix + (34.9435566)*stateseven 
	var action int
    	if armzero > armone {
		action = 0
	}else{
...
func GetPolicy(sch *scheduler, s *session) (int, []*path){
	packetNumber := make(map[protocol.PathID]uint64)
	retransNumber := make(map[protocol.PathID]uint64)
	...
	types.Output(cwnd[firstPath])/types.Output(protocol.DefaultTCPMSS)/300, types.Output(cwnd[secondPath])/types.Output(protocol.DefaultTCPMSS)/300, cwndlevel[firstPath], cwndlevel[secondPath], types.Output(BSend)/types.Output(protocol.DefaultTCPMSS)/30000}
